...
@Command('slots3')
export class Slots3KCommand extends CommandMessage {
  ...
  constructor(
    ...
    this.queueProcessor =
      new (class extends BaseQueueProcessor<ChannelMessage> {
        constructor(private slotsCommand: Slots3KCommand) {
          super('Slots3KCommand', 1, 25000);
        }
  ...
  async getTop5JackpotUsers() {
    const rawData = await this.jackPotTransaction
      ...
      .andWhere('jackpot.typeSlots = :typeSlots', { typeSlots: SlotsType.K3 })
      .groupBy('jackpot.user_id')
  ...
  public async processSlotMessage(message: ChannelMessage) {
    ...
    try {
      ...
      if (newJackPot < 555000) {
        this.scheduleJackpotFloorUpdate(555000, userKey);
      }
      ...
      if (win) {
        this.jackPotTransaction
          ...
          .then(async () => {
            ...
            const jackpotText = `nổ jackpot 3K ${isJackPot ? '777 ' : ''}${wonAmount.toLocaleString('vi-VN')}đ`;
            await Promise.all([
              user.sendDM({
